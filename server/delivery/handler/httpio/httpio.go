...
import (
	...
	"flatbuffer-explore/server/entities/params"
	"fmt"
	"net/http"
	"reflect"
	"strings"
...
func (f *formio) PrintRecv(outObject interface{}) {
	...
	path := fmt.Sprintf("%s %s", f.request.Method, f.request.URL.Path)
	_ = f.context.ShouldBindHeader(&header)
	go receiveForm("RECV", header, outObject, f.request.RemoteAddr, path)
}
...
func (f *formio) ResponseDataWithAbort(statusCode int, data []byte, logData interface{}, contentType string) {
	f.sendResponseData(statusCode, data, logData, contentType, true)
}

func (f *formio) ResponseData(statusCode int, data []byte, logData interface{}, contentType string) {
	f.sendResponseData(statusCode, data, logData, contentType, false)
}
